...
void filter(VectorXd &x, MatrixXd &P) {
	...
	for (unsigned int n = 0; n < measurements.size(); ++n) {
		...
		VectorXd z = measurements[n]; //for computing y
		//YOUR CODE HERE

		// KF Measurement update step
		VectorXd y = z - (H*x); // for computing new x             
		VectorXd s = (H*P) * H.transpose() + R; // for computing K (Kalmain Gain)
		VectorXd K = P * H.transpose() * s.inverse(); // for computing new x and new P
